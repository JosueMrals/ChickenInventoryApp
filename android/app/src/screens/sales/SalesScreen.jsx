...
import { View, Text, Animated, Dimensions, Alert } from 'react-native';
...
async function saveSaleRecord(saleData) {
    try {
        await firestore().collection('sales_operations').add(saleData);
        console.log('Sale record saved successfully!');
    } catch (error) {
        console.error("Error saving sale record: ", error);
        Alert.alert('Error', 'No se pudo guardar el registro de la venta.');
    }
}
...
export default function SalesScreen({ route }) {
    ...
    const handleSaleComplete = (saleData) => {
        saveSaleRecord(saleData);
        // optional: refresh or show toast
        Alert.alert('Venta completada', 'La venta ha sido registrada exitosamente.');
    };
