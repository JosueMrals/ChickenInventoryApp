...
export default function ProductQuantityScreen({ navigation, route }) {
  ...
  const renderInfoCarrusel = () => {
    ...
    if (wholesale.length === 0 && activeBonuses.length === 0 && categoryTiers.length === 0) return null;

    return (
      <View style={{ width: '100%', marginBottom: 4 }}>
        {categoryTiers.length > 0 && (
          <View style={{ marginBottom: (wholesale.length > 0 || activeBonuses.length > 0) ? 4 : 0 }}>
            <Text style={localStyles.chipSectionLabel}>Desc. {normalizeCategory(product.category)}:</Text>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={{ flex: 1 }}>
          ...
          <View style={localStyles.header}>
            ...
            <Text style={localStyles.headerTitle} numberOfLines={1}>{product.name}</Text>
            <TouchableOpacity onPress={clearQty} style={localStyles.headerBack}>
          ...
          <View style={localStyles.displayCard}>
            <View style={localStyles.displayRow}>
              ...
              <Text style={localStyles.displayUnitPrice}>C${(pricePreview.unitPrice || 0).toFixed(2)}</Text>
              {sourceLabel.label && (
          ...
          <View style={localStyles.footer}>
            ...
            <TouchableOpacity
              ...
              onPress={() => { onConfirm(0); navigation.goBack(); }}
              activeOpacity={0.85}
            >
              <Icon name="trash-outline" size={16} color="#DC2626" />
              <Text style={localStyles.removeBtnText}>Eliminar producto</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const localStyles = StyleSheet.create({
  ...
  headerBack: { width: 36, alignItems: 'center' },
  headerTitle: { flex: 1, fontSize: 16, fontWeight: '700', color: '#111827', textAlign: 'center', marginHorizontal: 4 },
  displayCard: {
    backgroundColor: '#fff', marginHorizontal: 14, marginTop: 12, marginBottom: 8,
    borderRadius: 16, paddingHorizontal: 16, paddingVertical: 14,
    elevation: 2, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 6, shadowOffset: { width: 0, height: 2 },
  },
  displayRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 10, gap: 8 },
  ...
  qtyBox: { flexDirection: 'row', alignItems: 'flex-end', gap: 4 },
  qtyText: { fontSize: 52, fontWeight: '800', color: '#111827', lineHeight: 58 },
  ...
  keyboardArea: { flex: 1, backgroundColor: '#F5F6FA', paddingHorizontal: 4 },
  chipSectionLabel: { fontSize: 10, color: '#6D28D9', fontWeight: '700', paddingHorizontal: 12, marginBottom: 2 },
  chip: { marginRight: 6, paddingHorizontal: 9, paddingVertical: 5, borderRadius: 20, borderWidth: 1, flexDirection: 'row', alignItems: 'center' },
  chipText: { fontSize: 10, fontWeight: '700' },
  footer: { paddingHorizontal: 14, paddingVertical: 10, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#F1F5F9', gap: 8 },
  confirmBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
    backgroundColor: '#007AFF', borderRadius: 14, paddingVertical: 14,
    elevation: 3, shadowColor: '#007AFF', shadowOpacity: 0.25, shadowRadius: 5, shadowOffset: { width: 0, height: 2 },
  },
  confirmBtnDisabled: { backgroundColor: '#93C5FD' },
  confirmBtnText: { color: '#fff', fontSize: 15, fontWeight: '700' },
  removeBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
    borderRadius: 12, paddingVertical: 10, borderWidth: 1, borderColor: '#FEE2E2', backgroundColor: '#FFF5F5',
  },
